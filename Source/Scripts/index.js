...
document.getElementById('match-btn').addEventListener('click', () => {
  ...
  db.collection('waiting').doc(user.uid).set({
...
function tryToMatch(user, preference) {
  if (!currentUserData || !currentUserData.sex) {
    showPage('home-page');
  ...
  db.collection('waiting').get().then(snap => {
    const matches = snap.docs.filter(doc => {
      ...
      const S = currentUserData.sex, P = preference;
...
document.getElementById('profile-form').addEventListener('submit', e => {
  ...
  db.collection('users').doc(user.uid).get().then(doc => {
    ...
    if (username !== currentUsername) {
      ...
      db.collection('users').doc(user.uid).update({ displayName }).then(() => {
        currentUserData.displayName = displayName;
        alert('Profile updated');
      });
    }
